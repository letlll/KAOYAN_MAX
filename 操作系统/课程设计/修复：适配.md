下面是针对您提供的代码进行修复的版本。主要修复内容如下：

1. **修正对 `action_type` 和 `action.target` 的处理顺序**：  
    在 `readRun()` 函数中先从文件中解析出 `action.action_type` 和 `action.target`，对其进行安全的修剪（trim）后，再将 `action` 压入 `program_actions[current_program]`，避免重复压入。
    
2. **去除重复的 `push_back` 调用**：  
    原代码中对 `program_actions[current_program].push_back(action);` 调用了两次，这是不必要的。
    
3. **确保在 "结束" 操作时不对 `action.target` 调用 `std::stoi`**。
    
4. **在 "跳转" 和 "读写磁盘" 操作前先检查 `action.target` 是否为数字字符串**，若不是则跳过 `std::stoi` 转换并报告错误。
    
5. **对空字符串或全空白字符串进行正确清理**：  
    利用 `trim_str` 函数在插入 `program_actions` 前对字符串进行处理。
    

以下为修复后的代码示例（仅展示修改过的关键部分，完整代码请参考您给出的原代码并进行替换）：
...
void Scheduler::readRun(const string& filename) {
    ...
    while (getline(infile, line)) {
        ...
        // 安全修剪 action_type 和 action.target
...
- 在 `readRun` 函数中对 `action_type` 和 `action.target` 进行安全修剪，并在修剪后才 `push_back` 到 `program_actions[current_program]`。
- 在处理动作时（FCFS、RoundRobin）根据 `action_type` 决定是否对 `action.target` 调用 `std::stoi`，并先检查 `isDigits`。
- 对于 "结束" 操作不执行数值转换，从而避免无效转换导致 `std::range_error`。

这样应能避免之前出现的 `std::range_error` 异常。