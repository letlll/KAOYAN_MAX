...
void rrScheduling() {
    ...
    while (!processQueue.empty() || index < processList.size()) {
        ...
        while (index < processList.size() && processList[index].createTime <= currentTime) {
            ...
            resultFile << " 进程: " << newProcess->pName << "在" << currentTime << " ms时被加入队列" << std::endl;
            index++;
        }
        ...
        if (currentProcess->startTime == -1) {
            ...
            resultFile << "进程 " << currentProcess->pName << "在" << currentTime << " ms时开始执行" << std::endl;
        }
        ...
        resultFile << "执行进程：" << currentProcess->pName
            << " | 执行时间：" << execTime << " ms" << std::endl;
        ...
        resultFile << "当前时间：" << currentTime << " ms |  "
            << currentProcess->pName << "剩余时间：" << remainingTimeMap[currentProcess->pName] << " ms" << std::endl;

        // If the process still has remaining time, re-enqueue it
        if (remainingTimeMap[currentProcess->pName] > 0) {
            processQueue.push(currentProcess);
            resultFile << "进程 " << currentProcess->pName << " 在 " << currentTime << " ms时重新加入队列" << std::endl;
        }
        else {
            ...
            resultFile << "进程 " << currentProcess->pName << " 在 " << currentTime << " ms"
    ...
    // Output final statistics for all processes
    for (const auto& process : processList) {
        resultFile << "进程名称: " << process.pName
            << ", 开始时间: " << process.startTime
            << ", 完成时间: " << process.completeTime
