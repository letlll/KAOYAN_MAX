...
- `email`：用于输入电子邮件地址，浏览器可进行基本的格式验证。
- `url`：用于输入 URL 地址，浏览器会验证格式。
- `tel`：用于输入电话号码，浏览器提供适当的键盘布局（移动设备上）。
- `date`：用于输入日期，显示日期选择器（根据浏览器支持）。
- `time`：用于输入时间，显示时间选择器（根据浏览器支持）。
- `range`：用于输入数值范围，通常显示一个滑动条。
- `search`：用于搜索框，表现上与文本框类似，但专门用于搜索输入。
